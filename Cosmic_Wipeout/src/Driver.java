...
public class Driver
{
    ...
    protected void playGame() {
        ...
        while (!user.isWinner() && !cpu.isWinner() && !discontinue)  //while there are no winners
        {                                                            //if its the users turn
            ...
            while (!cpu.isWinner() && !discontinue)
                takeATurn(cpu);
        }
    ...
    private boolean takeOneRoll(Player player)
    {
        ...
        if(!player.isWinner())                           //roll dice if the players score is not >= 500
        {
            ...
            int score = calcScore(roll, player);                 //Calculate value of roll
    ...
    private int calcScore(int[] roll, Player player)
    {
        if(player.equals(user))
            return humanScoreWizard.produceScore(roll);
        else
            return cpuScoreWizard.produceScore(roll);
    }

    private boolean discontinue()
    {
        Scanner scan = new Scanner(System.in);
        System.out.println("Would you like to continue (1), restart (2) or exit (3) the game");
